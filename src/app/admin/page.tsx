...
export default function AdminPage() {
  ...
  const [dbStatus, setDbStatus] = useState<{ connected: boolean; latencyMs?: number; tablesCount?: number } | null>(null);
  ...
  useEffect(() => {
    // 1. Cek jika sesi login aktif saat ini adalah akun Administrator
    const session = getAuthSession();
    if (session.isLoggedIn && (session.user?.role === "admin" || session.user?.email === "admin@cbt-pplg.sch.id")) {
    ...
    // 2. Cek koneksi live ke TiDB Serverless
    fetch("/api/db/health")
      .then((res) => res.json())
      .then((data) => {
        if (data.status === "connected") {
          setDbStatus({
            ...
            tablesCount: data.tables?.length || 8,
          });
        } else {
    ...
    >
      <div className="flex-1 bg-surface p-margin lg:p-space-xl overflow-y-auto min-h-screen">
        <div className="max-w-7xl mx-auto space-y-space-md">
          ...
          {/* Admin Hero Header Bar */}
          <section className="flex flex-col lg:flex-row lg:items-center justify-between gap-space-md bg-surface-container-lowest p-space-lg rounded-2xl border border-outline-variant shadow-elevation-1">
            <div>
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-primary/10 text-primary border border-primary/20 text-label-sm font-mono mb-space-xs font-semibold">
                <ShieldCheck className="w-3.5 h-3.5" />
                <span>Pusat Kendali &amp; Manajemen Ujian CBT-PPLG</span>
              </div>
              ...
              <p className="text-body-md text-on-surface-variant mt-1">
                Ruang Kontrol &amp; Server &bull; SMK Pusat Keunggulan PPLG
              </p>
            </div>

            <div className="flex items-center gap-2.5 flex-wrap self-start lg:self-auto">
              {/* TiDB Live Status Badge */}
              <div className="flex items-center gap-2.5 px-3.5 py-2 rounded-xl bg-surface-container-low border border-outline-variant shadow-elevation-1">
                <div className="relative flex items-center justify-center">
                  <span className={`w-2.5 h-2.5 rounded-full ${dbStatus?.connected ? "bg-emerald-500 animate-ping" : "bg-amber-500 animate-pulse"} absolute`} />
                ...
                <div className="text-left font-mono">
                  ...
                  <div className={`text-[11px] font-bold ${dbStatus?.connected ? "text-emerald-600 dark:text-emerald-400" : "text-amber-600"}`}>
                    {dbStatus?.connected ? `Online (${dbStatus.latencyMs}ms)` : "Connecting..."}
                  </div>
                </div>
              </div>
              ...
              <button
                ...
                className="px-3.5 py-2.5 rounded-xl border border-outline-variant bg-surface-container-low hover:bg-surface-container text-on-surface font-semibold text-body-sm transition-all flex items-center gap-1.5 shadow-elevation-1 active:scale-95"
                title="Ekspor seluruh nilai siswa ke Excel/CSV"
              >
              ...
              <button
                type="button"
                onClick={loadData}
                className="p-2.5 rounded-xl border border-outline-variant bg-surface-container-low hover:bg-surface-container text-on-surface transition-all active:scale-95"
              ...
              <button
                ...
                className="px-3.5 py-2.5 rounded-xl border border-red-500/30 bg-red-500/10 hover:bg-red-500/20 text-red-600 font-semibold text-body-sm transition-all flex items-center gap-1.5 shadow-elevation-1 active:scale-95"
